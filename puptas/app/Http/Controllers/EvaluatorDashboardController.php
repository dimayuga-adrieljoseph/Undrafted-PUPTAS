...
public function passApplication(Request $request, $userId)
{
    ...
    // Update status to next step (e.g., 'endorsed' or 'passed')
    $application->status = 'endorsed'; // or 'passed', depending on your flow
    $application->save();

    // Log evaluator's action
    ApplicationProcess::create([
        'application_id' => $application->id,
        'stage' => 'evaluator',
        'status' => 'endorsed', // match above
        'notes' => $request->note,
        'performed_by' => auth()->id(),
    ]);
