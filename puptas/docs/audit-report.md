...
### 🔴 Immediate (Critical)
- [x] ~~Remove all three `/debug-medical/*` routes from `web.php`~~ ✅ **VERIFIED: Not present in codebase**
- [x] ~~Remove or fix `CallbackController::handle()` (SSRF vulnerability)~~ ✅ **FIXED: Method disabled, returns 410 Gone**
- [x] ~~Rotate `IDP_CLIENT_SECRET` and all secrets that may have been exposed in `.env`~~ ✅ **NOT NEEDED: Verified `.env` never committed to git**
- [x] **ACTION REQUIRED:** Set `APP_DEBUG=false`, `SESSION_ENCRYPT=true`, and `SESSION_SECURE_COOKIE=true` in Railway production environment variables (see `docs/SECURITY_CHECKLIST.md`)
...
- [x] Set `SESSION_SECURE_COOKIE=true` in production
- [ ] Add webhook replay protection (timestamp + nonce check)
- [ ] Move Tesseract OCR to a queued job
- [x] Switch `QUEUE_CONNECTION` to `database` or `redis` in production
