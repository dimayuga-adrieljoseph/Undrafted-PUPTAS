                  ...
                  <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 border border-gray-200 dark:border-gray-600 hover:border-maroon-500 transition-all">
                    ...
                    <div v-if="activeUploadKey !== key">
                      <button
                        ...
                        @click.prevent="triggerFileInput(key); openInlineUpload(key)"
                    ...
                    <div v-else class="mt-3 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 via-white to-gray-100 p-3 shadow-sm dark:border-gray-700 dark:from-gray-950/70 dark:via-gray-900 dark:to-gray-950/80">
                      ...
                      <div v-if="activeUploadKey === key" class="mt-3 space-y-3">
                        <div v-if="activeUploadFile" class="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 px-3 py-3 text-left dark:border-blue-800 dark:bg-blue-900/20">
                          <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-200">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l7.071-7.071a4 4 0 10-5.657-5.657l-7.07 7.071a6 6 0 108.485 8.485L20.5 13"></path>
                            </svg>
                          </div>
                          <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-semibold text-blue-900 dark:text-blue-200">{{ activeUploadFile.name }}</p>
                            <p class="mt-0.5 text-xs text-blue-700/90 dark:text-blue-300/90">
                              {{ activeUploadFile.type || 'Image file' }}<span v-if="formatFileSize(activeUploadFile.size)"> · {{ formatFileSize(activeUploadFile.size) }}</span>
                            </p>
                          </div>
                          <span class="rounded-full bg-blue-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-blue-700 dark:bg-blue-900/40 dark:text-blue-200">
                            Ready
                          </span>
                        </div>

                        <div v-if="activeUploadProgress && activeUploadKey === key" class="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900/70">
                          <div class="mb-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                            <span>Uploading</span>
                            <span>{{ activeUploadProgress }}%</span>
                          </div>
                          <div class="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700">
                            <div class="h-2 rounded-full bg-[#9E122C] transition-all duration-300" :style="{ width: activeUploadProgress + '%' }"></div>
                          </div>
                        </div>
                        ...
                        <div class="flex gap-2">
                          ...
                          >
                            {{ activeUploadSuccess ? 'Close' : 'Cancel' }}
                          </button>
                          <button
                            v-if="activeUploadSuccess"
                            class="flex-1 rounded-xl bg-green-600 px-3 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-green-700"
                            :disabled="activeUploadUploading"
                            @click="closeInlineUpload"
                          >
                            Done
                          ...
                          <button
                            v-else
                            class="flex-1 rounded-xl bg-[#9E122C] px-3 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-[#b51834] disabled:opacity-70"
                            :disabled="activeUploadUploading"
                            @click="document.getElementById('inline-file-input-' + key)?.click()"
                          >
                            {{ activeUploadUploading ? 'Uploading...' : 'Choose another file' }}
                          </button>
                        </div>
                      </div>
                    </div>
