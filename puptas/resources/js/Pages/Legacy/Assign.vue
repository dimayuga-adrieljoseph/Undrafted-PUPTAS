<template>
  <Head title="Create & Assign Users" />
  <AppLayout>
    <div class="container">
      <!-- Form Card -->
      <div class="form-card">
        ...
        <!-- Success Message -->
        <div v-if="$page.props.flash.success" class="success-snackbar show">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div style="text-align: right; margin-top: 1.5rem;">
            <button type="submit" class="form-button" :disabled="form.processing">
              <i class="fas fa-user-plus"></i> {{ form.processing ? 'CREATING...' : 'CREATE' }}
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="form-card">
        ...
        <p v-if="assignedUsers.length === 0" style="text-align:center; color:#666;">
        ...
        <table v-else>
          ...
          <tbody>
            <tr v-for="user in assignedUsers" :key="user.id">
              ...
              <td>
                <span v-if="user.programs.length === 0">Not assigned</span>
              ...
              <td>
                <div class="action-container">
                  ...
                  <button @click="confirmDelete(user.id)" class="action-button delete-button">
...
const deleteUser = () => {
  router.delete(route('admin.users.delete', userToDelete.value), {
    preserveScroll: true,
    onSuccess: () => {
      showDeleteModal.value = false;
      userToDelete.value = null;
